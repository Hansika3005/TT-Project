...
import { Search, Plus, UserPlus, Star } from "lucide-react";
...
export default function Agents() {
  ...
  const onSubmit = async (data) => {
    await createAgent({ ...data, status: "ACTIVE", totalDeliveries: 0, rating: 5.0 });
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          <h1 className="text-3xl font-black tracking-tight">Delivery Agents</h1>
          <p className="text-sm text-muted-foreground mt-0.5 font-medium">Manage personnel and performance</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/40 bg-muted/30">
                {["#", "Agent", "Email", "Deliveries", "Rating", "Status"].map((h) => (
              ...
              {isLoading ? (
                    ...
                    <div className="flex flex-col items-center gap-3 text-muted-foreground">
                      ...
                      <div>
                        ...
                        <p className="text-xs mt-1 opacity-60">
                          {!search && "Add your first delivery agent using the button above"}
                        </p>
                      </div>
                    </div>
                  </td>
                ...
                filtered.map((agent, idx) => (
                  <motion.tr key={agent.id}
                    ...
                    <td className="px-4 py-3.5 font-mono text-xs text-muted-foreground">#{agent.id}</td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2.5">
                        <div className="h-8 w-8 rounded-full bg-gradient-to-br from-emerald-500/60 to-teal-500/60
                    ...
                    <td className="px-4 py-3.5 text-muted-foreground text-xs">{agent.email || "—"}</td>
                    <td className="px-4 py-3.5 font-bold">{agent.totalDeliveries || 0}</td>
                    <td className="px-4 py-3.5">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Delivery Agent">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-border/40">
            ...
            <Button type="submit"><UserPlus className="h-4 w-4" /> Add Agent</Button>
          </div>
        </form>
      </Modal>
    </div>
